...
namespace Grad_Api.Services
{
    public class QuizService : IQuizService
    {
        ...
        public QuizService(IQuizRepository quizRepository, ILogger<QuizService> logger, HttpClient httpClient)
        {
        ...
        public async Task<bool> AddQuestionToExcelAsync(AddQuestionDto newQuestion, string csvFilePath)
        {
            try
            {
                ...
                {
            EscapeCsv(newQuestion.QuestionText),
            EscapeCsv(newQuestion.OptionA),
            EscapeCsv(newQuestion.OptionB),
            EscapeCsv(newQuestion.OptionC),
            EscapeCsv(newQuestion.OptionD),
            EscapeCsv(newQuestion.CorrectAnswer),
            EscapeCsv(newQuestion.Defficulty),
            EscapeCsv(newQuestion.Subject),
            EscapeCsv(newQuestion.CourseCategory),
            newQuestion.UnitNumber.ToString()
                });
                ...
                using (var writer = new StreamWriter(csvFilePath, append: true, Encoding.UTF8))
                {
                    if (!fileExists)
                    {
                        var headerLine = "QuestionText,OptionA,OptionB,OptionC,OptionD,CorrectAnswer,Defficulty,Subject,CourseCategory,UnitNumber";
        ...
        public async Task<string> PredictDifficultyAsync(QuestionDefecultDto question)
        {
            var response = await _httpClient.PostAsJsonAsync("http://127.0.0.1:5001/predict_difficulty", question);

            if (!response.IsSuccessStatusCode)
            {
                throw new Exception("Error calling the prediction API.");
            }
